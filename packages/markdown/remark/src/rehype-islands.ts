...
// `unist`/`remark`/`rehype` (not sure) often generate malformed HTML inside of <wromo-island>
...
export default function rehypeIslands(): any {
	return function (node: any): any {
		return visit(node, 'element', (el) => {
			// Bugs only happen inside of <wromo-island> islands
			if (el.tagName == 'wromo-island') {
