...
const remarkShiki = async (
	...
	return () => (tree: any) => {
		visit(tree, 'code', (node) => {
			...
			// Replace "shiki" class naming with "wromo" and add "is:raw".
			html = html.replace(
				'<pre class="shiki"',
				`<pre is:raw class="wromo-code${scopedClassName ? ' ' + scopedClassName : ''}"`
			);
			// Replace "shiki" css variable naming with "wromo".
			html = html.replace(
				/style="(background-)?color: var\(--shiki-/g,
				'style="$1color: var(--wromo-code-'
			);
