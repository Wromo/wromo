...
export interface WromoTelemetryOptions {
	version: string;
}
...
export class WromoTelemetry {
	...
	private debug = debug('wromo:telemetry');

	private get wromoVersion() {
		return this.opts.version;
	}
	private get WROMO_TELEMETRY_DISABLED() {
		return process.env.WROMO_TELEMETRY_DISABLED;
	}
	private get TELEMETRY_DISABLED() {
		return process.env.TELEMETRY_DISABLED;
	}

	constructor(private opts: WromoTelemetryOptions) {
		// TODO: When the process exits, flush any queued promises
		// This line caused a "cannot exist wromo" error, needs to be revisited.
	...
	// Create a ONE-WAY hash so there is no way for Wromo to decode the value later.
	...
	private get isDisabled(): boolean {
		if (Boolean(this.WROMO_TELEMETRY_DISABLED || this.TELEMETRY_DISABLED)) {
			return true;
		}
	...
	async record(event: TelemetryEvent | TelemetryEvent[] = []) {
		...
		const meta = getAnonymousMeta(this.wromoVersion);
