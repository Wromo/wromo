...
describe('Error display', () => {
	 ...
	 */
	describe.skip('Wromo template syntax', async () => {
		let devServer;
		...
		it('properly detect syntax errors in template', async () => {
			let html = await fixture.fetch('/wromo-syntax-error').then((res) => res.text());
			...
			await fixture.editFile('./src/pages/wromo-syntax-error.wromo', `<h1>No syntax error</h1>`);

			// 3. Verify that the file is fixed.
			html = await fixture.fetch('/wromo-syntax-error').then((res) => res.text());
			$ = cheerio.load(html);
		...
		it('shows useful error when frontmatter import is not found', async () => {
			...
			let $ = cheerio.load(html);
			expect($('.statusMessage').text()).to.equal('Internal Error');
			expect($('.error-message').text()).to.equal('failed to load module for ssr: ../abc.wromo');

			// 2. Edit the file, fixing the error
			await fixture.editFile('./src/pages/import-not-found.wromo', '<h1>No import error</h1>');
