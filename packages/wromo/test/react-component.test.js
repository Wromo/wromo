...
describe('React Components', () => {
	...
	describe('build', () => {
		...
		it('Can load React', async () => {
			...
			expect($('wromo-island[uid]')).to.have.lengthOf(5);

			// test 9: Check island deduplication
			const uniqueRootUIDs = new Set($('wromo-island').map((i, el) => $(el).attr('uid')));
    ...
    https://docs.wromo.build/reference/api-reference/#importmeta`
			);
		});
