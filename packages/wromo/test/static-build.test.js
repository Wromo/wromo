...
function removeBasePath(path) {
	// `/subpath` is defined in the test fixture's Wromo config
...
describe('Static build', () => {
	...
	it('Builds out .wromo pages', async () => {
		const html = await fixture.readFile('/index.html');
		expect(html).to.be.a('string');
	});

	it('can build pages using Wromo.glob()', async () => {
		const html = await fixture.readFile('/index.html');
	...
	it('Builds out .json files', async () => {
		...
		expect(content.name).to.equal('Wromo Technology Company');
		expect(content.url).to.equal('https://wromo.build/');
	});
