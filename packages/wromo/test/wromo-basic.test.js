...
describe('Wromo basics', () => {
	let fixture;
	let previewServer;

	before(async () => {
		fixture = await loadFixture({
			root: './fixtures/wromo-basic/',
		});
		await fixture.build();
	...
	describe('build', () => {
		...
		it('Allows scoped classes with spread', async () => {
			...
			expect($('#spread-plain').attr('class')).to.match(/wromo-.*/);

			expect($('#spread-class')).to.have.lengthOf(1);
			expect($('#spread-class').attr('class')).to.match(/wromo-.*/);

			expect($('#spread-class-list')).to.have.lengthOf(1);
			expect($('#spread-class-list').attr('class')).to.match(/wromo-.*/);
		});
