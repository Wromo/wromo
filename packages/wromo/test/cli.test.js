...
describe('wromo cli', () => {
	const cliServerLogSetupWithFixture = (flags, cmd) => {
		const projectRootURL = new URL('./fixtures/wromo-basic/', import.meta.url);
	...
	it('wromo dev welcome', async () => {
		...
		expect(messages[0]).to.contain('wromo');
	...
	['dev', 'preview'].forEach((cmd) => {
		...
		networkLogFlags.forEach(([flag, flagValue]) => {
			it(`wromo ${cmd} ${flag} ${flagValue ?? ''} - network log`, async () => {
		...
		hostToExposeFlags.forEach(([flag, flagValue]) => {
			it(`wromo ${cmd} ${flag} ${flagValue} - host to expose`, async () => {
		...
		noNetworkLogFlags.forEach(([flag, flagValue]) => {
			it(`wromo ${cmd} ${flag} ${flagValue} - no network log`, async () => {
				const { local, network } = await cliServerLogSetupWithFixture([flag, flagValue], cmd);
...
describe('wromo cli i18n', () => {
	const LOCALES = ['en_US', 'sv_SE', 'es_419.UTF-8', 'es_ES@euro', 'C'];
	LOCALES.forEach((locale) => {
		it(`wromo does NOT throw on "${locale}" locales`, async () => {
			const projectRootURL = new URL('./fixtures/wromo-basic/', import.meta.url);
			let error = null;
			try {
