...
describe('PostCSS', () => {
	...
	it('works in Wromo page styles', () => {
		expect(bundledCSS).to.match(new RegExp(`\.wromo-page(\.(\w|-)*)*\.nested`));
	});

	it('works in Wromo component styles', () => {
		expect(bundledCSS).to.match(new RegExp(`\.wromo-component(\.(\w|-)*)*\.nested`));
	});
