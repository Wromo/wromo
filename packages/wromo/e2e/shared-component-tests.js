...
export function prepareTestFactory(opts) {
	...
	const createTests = ({ pageUrl, pageSourceFilePath, componentFilePath, counterCssFilePath }) => {
		test('server only', async ({ page, wromo }) => {
			await page.goto(wromo.resolveUrl(pageUrl));
		...
		test('client:idle', async ({ page, wromo }) => {
			await page.goto(wromo.resolveUrl(pageUrl));
		...
		test('client:load', async ({ page, wromo }) => {
			await page.goto(wromo.resolveUrl(pageUrl));
		...
		test('client:visible', async ({ page, wromo }) => {
			await page.goto(wromo.resolveUrl(pageUrl));
		...
		test('client:media', async ({ page, wromo }) => {
			await page.goto(wromo.resolveUrl(pageUrl));
		...
		test('client:only', async ({ page, wromo }) => {
			await page.goto(wromo.resolveUrl(pageUrl));
		...
		test('HMR', async ({ page, wromo }) => {
			...
			await wromo.editFile(pageSourceFilePath, (original) =>
			...
			await wromo.editFile(componentFilePath, (original) =>
				original.replace(
			...
			await wromo.editFile(counterCssFilePath || './src/components/Counter.css', (original) =>
