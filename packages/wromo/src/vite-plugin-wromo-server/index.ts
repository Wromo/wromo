...
import type { WromoConfig, ManifestData } from '../@types/wromo';
...
async function writeWebResponse(res: http.ServerResponse, webResponse: Response) {
	...
	if (body) {
		if (Symbol.for('wromo.responseBody') in webResponse) {
			let stream = (webResponse as any)[Symbol.for('wromo.responseBody')];
...
function getCustom404Route(config: WromoConfig, manifest: ManifestData) {
	// For Windows compat, use relative page paths to match the 404 route
	const relPages = resolvePages(config).href.replace(config.root.href, '');
	const pattern = new RegExp(`${appendForwardSlash(relPages)}404.(wromo|md)`);
...
/** The main logic to route dev server requests to pages in Wromo. */
async function handleRequest(
	...
	try {
		...
		const preloadedComponent = await preload({ wromoConfig: config, filePath, viteServer });
 ...
 * since they are inside of a link tag for Wromo.
 */
const forceTextCSSForStylesMiddleware: vite.Connect.NextHandleFunction = function (req, res, next) {
	if (req.url) {
		...
		const url = new URL(req.url, 'https://wromo.build');
		// lang.css is a search param that exists on Wromo, Svelte, and Vue components.
		// We only want to override for wromo files.
		if (url.searchParams.has('wromo') && url.searchParams.has('lang.css')) {
...
export default function createPlugin({ config, logging }: WromoPluginOptions): vite.Plugin {
	return {
		name: 'wromo:server',
