...
function getPrivateEnv(viteConfig: vite.ResolvedConfig, wromoConfig: WromoConfig) {
	...
	const fullEnv = loadEnv(
		viteConfig.mode,
		viteConfig.envDir ?? fileURLToPath(wromoConfig.root),
		''
	);
	...
	return {
		...
		async transform(source, id, options) {
			...
			if (typeof privateEnv === 'undefined') {
				privateEnv = getPrivateEnv(config, wromoConfig);
				if (privateEnv) {
					privateEnv.SITE = wromoConfig.site ? `'${wromoConfig.site}'` : 'undefined';
