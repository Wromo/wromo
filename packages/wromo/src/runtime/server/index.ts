...
async function* _render(child: any): AsyncIterable<any> {
	...
	// Maybe create clearly named function for what this is doing.
		...
		child instanceof WromoComponent ||
		Object.prototype.toString.call(child) === '[object WromoComponent]'
	) {
		yield* renderWromoComponent(child);
...
function isWromoComponent(obj: any): obj is WromoComponent {
	return (
		typeof obj === 'object' && Object.prototype.toString.call(obj) === '[object WromoComponent]'
	);
}

export async function render(htmlParts: TemplateStringsArray, ...expressions: any[]) {
	return new WromoComponent(htmlParts, expressions);
}

// The callback passed to to $$createComponent
export interface WromoComponentFactory {
	(result: any, props: any, slots: any): ReturnType<typeof render> | Response;
	isWromoComponentFactory?: boolean;
}

// Used in creating the component. aka the main export.
export function createComponent(cb: WromoComponentFactory) {
	// Add a flag to this callback to mark it as an Wromo component
	// INVESTIGATE does this need to cast
	(cb as any).isWromoComponentFactory = true;
	return cb;
}
...
export const Fragment = Symbol('Wromo.Fragment');

function guessRenderers(componentUrl?: string): string[] {
	const extname = componentUrl?.split('.').pop();
	switch (extname) {
		case 'svelte':
			return ['@wromojs/svelte'];
		case 'vue':
			return ['@wromojs/vue'];
		case 'jsx':
		case 'tsx':
			return ['@wromojs/react', '@wromojs/preact'];
		default:
			return ['@wromojs/react', '@wromojs/preact', '@wromojs/vue', '@wromojs/svelte'];
	}
}
...
export async function renderComponent(
	...
	if (Component && (Component as any).isWromoComponentFactory) {
		async function* renderWromoComponentInline(): AsyncGenerator<string, void, undefined> {
		...
		return renderWromoComponentInline();
	}
	...
	const metadata: WromoComponentMetadata = { displayName };
	...
	if (hydration) {
		metadata.hydrate = hydration.directive as WromoComponentMetadata['hydrate'];
...
There are no \`integrations\` set in your \`wromo.config.mjs\` file.
	...
	if (metadata.hydrate !== 'only') {
		...
		if (metadata.hydrateArgs) {
			...
			renderer = renderers.filter(
				({ name }) => name === `@wromojs/${rendererName}` || name === rendererName
			)[0];
		}
		...
		if (!renderer) {
			...
			renderer = renderers.filter(
				({ name }) => name === `@wromojs/${extname}` || name === extname
			)[0];
		}
	}
...
Using the \`client:only\` hydration strategy, Wromo needs a hint to use the correct renderer.
Did you mean to pass <${metadata.displayName} client:only="${probableRendererNames
				.map((r) => r.replace('@wromojs/', ''))
				.join('|')}" />
`);
...
There ${plural ? 'are' : 'is'} ${renderers.length} renderer${
					plural ? 's' : ''
				} configured in your \`wromo.config.mjs\` file,
...
but Wromo encountered an error during server-side rendering.
...
If you're still stuck, please open an issue on GitHub or join us at https://wromo.build/chat.`);
			}
		}
	} else {
		if (metadata.hydrate === 'only') {
	...
	if (!html && typeof Component === 'string') {
		...
		const iterable = renderWromoComponent(
	...
	if (!hydration) {
		if (isPage || renderer?.name === 'wromo:jsx') {
			return html;
		}
		return markHTMLString(html.replace(/\<\/?wromo-slot\>/g, ''));
	}

	// Include componentExport name, componentUrl, and props in hash to dedupe identical islands
	const wromoId = shorthash(
	...
	const island = await generateHydrateScript(
		{ renderer: renderer!, result, wromoId, props },
		metadata as Required<WromoComponentMetadata>
	);

	// Render template if not all wromo fragments are provided.
	let unrenderedSlots: string[] = [];
	if (html) {
		if (Object.keys(children).length > 0) {
			for (const key of Object.keys(children)) {
				if (!html.includes(key === 'default' ? `<wromo-slot>` : `<wromo-slot name="${key}">`)) {
	...
	const template =
		unrenderedSlots.length > 0
			? unrenderedSlots
					.map(
						(key) =>
							`<template data-wromo-template${key !== 'default' ? `="${key}"` : ''}>${
								children[key]
							}</template>`
					)
					.join('')
	...
	return markHTMLString(prescripts + renderElement('wromo-island', island, false));
}

/** Create the Wromo.fetchContent() runtime function. */
function createDeprecatedFetchContentFn() {
	return () => {
		throw new Error('Deprecated: Wromo.fetchContent() has been replaced with Wromo.glob().');
	};
}

/** Create the Wromo.glob() runtime function. */
function createWromoGlobFn() {
	const globHandler = (importMetaGlobResult: Record<string, any>, globValue: () => any) => {
		let allEntries = [...Object.values(importMetaGlobResult)];
		if (allEntries.length === 0) {
			throw new Error(`Wromo.glob(${JSON.stringify(globValue())}) - no matches found.`);
		}
	...
	return globHandler as unknown as WromoGlobalPartial['glob'];
}

// This is used to create the top-level Wromo global; the one that you can use
// Inside of getStaticPaths.
export function createWromo(
	filePathname: string,
	_site: string,
...
export function addAttribute(value: any, key: string, shouldEscape = true) {
	...
	if (STATIC_DIRECTIVES.has(key)) {
		...
		console.warn(`[wromo] The "${key}" directive cannot be applied dynamically at runtime. It will not be rendered as an attribute.
...
export async function renderToString(
	result: SSRResult,
	componentFactory: WromoComponentFactory,
	props: any,
	children: any
): Promise<string> {
	const Component = await componentFactory(result, props, children);

	if (!isWromoComponent(Component)) {
		const response: Response = Component;
		throw response;
	}

	let html = '';
	for await (const chunk of renderWromoComponent(Component)) {
		html += chunk;
	}
	return html;
}
...
export async function renderToIterable(
	...
	return renderWromoComponent(Component);
}
...
export async function renderPage(
	...
	if (isWromoComponent(factoryReturnValue)) {
		let iterable = renderWromoComponent(factoryReturnValue);
...
// This function is called by Wromo components that do not contain a <head> component
// This accomodates the fact that using a <head> is optional in Wromo, so this
...
export async function* renderWromoComponent(
	component: InstanceType<typeof WromoComponent>
...
function renderElement(
	...
	const { lang: _, 'data-wromo-id': wromoId, 'define:vars': defineVars, ...props } = _props;
	if (defineVars) {
		if (name === 'style') {
			if (props['is:global']) {
				children = defineStyleVars(`:root`, defineVars) + '\n' + children;
			} else {
				children = defineStyleVars(`.wromo-${wromoId}`, defineVars) + '\n' + children;
			}
