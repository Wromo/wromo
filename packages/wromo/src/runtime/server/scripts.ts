import type { SSRResult } from '../../@types/wromo';
...
import islandScript from './wromo-island.prebuilt.js';
...
export function getPrescripts(type: PrescriptType, directive: string): string {
	...
	// an wromo-island element the callbacks will fire immediately, causing the JS
	// deps to be loaded immediately.
	switch (type) {
		case 'both':
			return `<style>wromo-island,wromo-slot{display:contents}</style><script>${
