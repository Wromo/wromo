if (import.meta.hot) {
	...
	async function updatePage() {
		...
		for (const root of doc.querySelectorAll('wromo-island')) {
			const uid = root.getAttribute('uid');
			const current = document.querySelector(`wromo-island[uid="${uid}"]`);
			if (current) {
				current.setAttribute('data-persist', '');
				root.replaceWith(current);
			}
		}
		// both Vite and Wromo's HMR scripts include `type="text/css"` on injected
		// <style> blocks. These style blocks would not have been rendered in Wromo's
		...
		return diff(document, doc).then(() => {
			...
			for (const root of document.querySelectorAll('wromo-island[data-persist]')) {
	...
	async function updateAll(files: any[]) {
		let hasWromoUpdate = false;
		let styles = [];
		for (const file of files) {
			if (file.acceptedPath.endsWith('.wromo')) {
				hasWromoUpdate = true;
				continue;
			}
			if (file.acceptedPath.includes('svelte&type=style')) {
				...
				// styles injected by Wromo for the component in favor of Vite's internal HMR.
				const injectedStyle = document.querySelector(
					`style[data-wromo-injected="${file.acceptedPath}"]`
				);
				if (injectedStyle) {
			...
			if (file.acceptedPath.includes('wromo&type=style')) {
				styles.push(
