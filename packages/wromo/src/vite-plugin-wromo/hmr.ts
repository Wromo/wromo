...
export async function handleHotUpdate(ctx: HmrContext, config: WromoConfig, logging: LogOptions) {
	// Invalidate the compilation cache so it recompiles
	invalidateCompilation(config, ctx.file);

	// go through each of these modules importers and invalidate any .wromo compilation
	...
	for (const mod of ctx.modules) {
		...
		if (mod.id?.endsWith('.wromo?html-proxy&index=0.js')) {
			filtered.delete(mod);
			continue;
		}
	...
	// Invalidate happens as a separate step because a single .wromo file
	...
	for (const mod of mods) {
		for (const imp of mod.importedModules) {
			if (imp.id?.includes('?wromo&type=script')) {
				mods.push(imp);
			}
		}
	}
	...
	if (isSelfAccepting) {
		info(logging, 'wromo', msg.hmr({ file }));
	} else {
		info(logging, 'wromo', msg.reload({ file }));
	}
