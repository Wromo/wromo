...
/**
 * Note: this is currently needed because Wromo is directly using a Vite internal CSS transform. This gives us
 ...
 *
 * In the future, we should remove this workaround and most likely implement our own Wromo style handling without
...
const configCache = new WeakMap<WromoConfig, CompilationCache>();
...
export function isCached(config: WromoConfig, filename: string) {
	return configCache.has(config) && configCache.get(config)!.has(filename);
}

export function invalidateCompilation(config: WromoConfig, filename: string) {
