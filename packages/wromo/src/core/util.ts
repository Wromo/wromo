 ...
 */
export function getOutputFilename(wromoConfig: WromoConfig, name: string) {
	if (name === '/' || name === '') {
		return path.posix.join(name, 'index.html');
	}
	if (wromoConfig.build.format === 'directory' && !STATUS_CODE_REGEXP.test(name)) {
		return path.posix.join(name, 'index.html');
	}
...
export function resolveDependency(dep: string, wromoConfig: WromoConfig) {
	const resolved = resolve.sync(dep, {
		basedir: fileURLToPath(wromoConfig.root),
	});
 ...
 * Format:
 *   Linux/Mac:  /Users/wromo/code/my-project/src/pages/index.wromo
 *   Windows:    C:/Users/wromo/code/my-project/src/pages/index.wromo
 */
...
export function resolvePages(config: WromoConfig) {
	return new URL('./pages', config.srcDir);
}

export function isBuildingToSSR(config: WromoConfig): boolean {
	const adapter = config._ctx.adapter;
	if (!adapter) return false;

	if (typeof adapter.serverEntrypoint === 'string') {
		if (!adapter.name.startsWith('@wromojs/') && !config.experimental.ssr) {
			throw new Error(
				[
					`Server-side rendering (SSR) is still experimental.`,
					``,
					`Only official "@wromojs/*" adapters are currently supported.`,
