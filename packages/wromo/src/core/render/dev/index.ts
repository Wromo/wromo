...
export interface SSROptions {
	/** an instance of the WromoConfig */
...
export async function loadRenderers(
	...
	return Promise.all(wromoConfig._ctx.renderers.map((r) => loadRenderer(viteServer, r)));
}
...
export async function preload({
	...
	filePath,
	viteServer,
}: Pick<SSROptions, 'wromoConfig' | 'filePath' | 'viteServer'>): Promise<ComponentPreload> {
	// Important: This needs to happen first, in case a renderer provides polyfills.
	const renderers = await loadRenderers(viteServer, wromoConfig);
...
export async function render(
	...
	[...stylesMap].forEach(([url, content]) => {
		...
		styles.add({
			props: {
				'data-wromo-injected': svelteStylesRE.test(url) ? url : true,
			},
			children: content,
		});
	});
	...
	let response = await coreRender({
		...
		pathname,
		scripts,
		// Resolves specifiers in the inline hydrated scripts, such as "@wromojs/preact/client.js"
		...
		route,
		routeCache,
		site: wromoConfig.site ? new URL(wromoConfig.base, wromoConfig.site).toString() : undefined,
		ssr: isBuildingToSSR(wromoConfig),
	});
