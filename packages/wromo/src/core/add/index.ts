import type { WromoTelemetry } from '@wromojs/telemetry';
...
export default async function add(names: string[], { cwd, flags, logging, telemetry }: AddOptions) {
	...
	if (configURL?.pathname.endsWith('package.json')) {
		throw new Error(
			`Unable to use wromo add with package.json#wromo configuration! Try migrating to \`wromo.config.mjs\` and try again.`
		);
	}
	applyPolyfill();
	...
	if (configURL) {
		...
		configURL = new URL('./wromo.config.mjs', appendForwardSlash(root.href));
	...
	try {
		ast = await parseWromoConfig(configURL);

		debug('add', 'Parsed wromo config');
		...
		ensureImport(
			...
			t.importDeclaration(
				...
				t.stringLiteral('wromo/config')
			)
		);
		wrapDefaultExport(ast, defineConfig);

		debug('add', 'Wromo config ensured `defineConfig`');

		for (const integration of integrations) {
			await addIntegration(ast, integration);
			debug('add', `Wromo config added integration ${integration.id}`);
		}
	} catch (err) {
		debug('add', 'Error parsing/modifying wromo config: ', err);
	...
	if (ast) {
		try {
			configResult = await updateWromoConfig({ configURL, ast, flags, logging });
		} catch (err) {
			debug('add', 'Error updating wromo config', err);
			throw createPrettyError(err as Error);
		}
	}
			...
			if (integrations.find((integration) => integration.id === 'tailwind')) {
				...
				if (!alreadyConfigured) {
					info(
						...
						null,
						`\n  ${magenta(
							`Wromo will generate a minimal ${bold('./tailwind.config.cjs')} file.`
						)}\n`
					);
...
async function parseWromoConfig(configURL: URL): Promise<t.File> {
	const source = await fs.readFile(fileURLToPath(configURL), { encoding: 'utf-8' });
	const result = parse(source);

	if (!result) throw new Error('Unknown error parsing wromo config');
	if (result.errors.length > 0)
		throw new Error('Error parsing wromo config: ' + JSON.stringify(result.errors));
...
function createPrettyError(err: Error) {
	err.message = `Wromo could not update your wromo.config.js file safely.
Reason: ${err.message}

You will need to add these integration(s) manually.
Documentation: https://docs.wromo.build/en/guides/integrations-guide/`;
	return err;
}
...
async function updateWromoConfig({
	configURL,
	ast,
	flags,
	logging,
	...
	const comment = '// https://wromo.build/config';
	...
	info(
		logging,
		null,
		`\n  ${magenta('Wromo will make the following changes to your config file:')}\n${message}`
	);

	if (await askToContinue({ flags })) {
		await fs.writeFile(fileURLToPath(configURL), output, { encoding: 'utf-8' });
		debug('add', `Updated wromo config`);
...
async function tryToInstallIntegrations({
	...
	if (installCommand === null) {
		...
		info(
			logging,
			null,
			`\n  ${magenta('Wromo will run the following command:')}\n  ${dim(
...
export async function validateIntegrations(integrations: string[]): Promise<IntegrationInfo[]> {
	...
	const integrationEntries = await Promise.all(
		integrations.map(async (integration): Promise<IntegrationInfo> => {
			...
			// Allow third-party integrations starting with `wromo-` namespace
			if (!name.startsWith('wromo-')) {
				scope = `wromojs`;
			}
