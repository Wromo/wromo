...
import type { WromoConfig, BuildConfig, ManifestData } from '../../@types/wromo';
...
/** `wromo build` */
export default async function build(config: WromoConfig, options: BuildOptions): Promise<void> {
	applyPolyfill();
	const builder = new WromoBuilder(config, options);
	await builder.run();
}

class WromoBuilder {
	private config: WromoConfig;
	...
	constructor(config: WromoConfig, options: BuildOptions) {
		if (options.mode) {
	...
	private async setup() {
		...
		const viteConfig = await createVite(
			...
			{ wromoConfig: this.config, logging, mode: 'build' }
		);
