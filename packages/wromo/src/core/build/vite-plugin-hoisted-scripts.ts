...
function virtualHoistedEntry(id: string) {
	return id.startsWith('/wromo/hoisted.js?q=');
}
...
export function vitePluginHoistedScripts(
	...
	return {
		name: '@wromo/rollup-plugin-wromo-hoisted-scripts',
		...
		async generateBundle(_options, bundle) {
			let assetInlineLimit = wromoConfig.vite?.build?.assetsInlineLimit || 4096;
			...
			for (const [id, output] of Object.entries(bundle)) {
				if (
					...
					for (const pathname of pages) {
						const vid = viteID(new URL('.' + pathname, wromoConfig.root));
