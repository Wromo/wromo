...
function shouldSkipDraft(pageModule: ComponentInstance, wromoConfig: WromoConfig): boolean {
	return (
		// Drafts are disabled
		!wromoConfig.markdown.drafts &&
...
// ie, src/pages/index.wromo instead of /Users/name..../src/pages/index.wromo
export function rootRelativeFacadeId(facadeId: string, wromoConfig: WromoConfig): string {
	return facadeId.slice(fileURLToPath(wromoConfig.root).length);
}
...
export function chunkIsPage(
	...
	if (chunk.facadeModuleId) {
		const facadeToEntryId = prependForwardSlash(
			rootRelativeFacadeId(chunk.facadeModuleId, wromoConfig)
		);
...
export async function generatePages(
	...
	const ssr = isBuildingToSSR(opts.wromoConfig);
	const serverEntry = opts.buildConfig.serverEntry;
	const outFolder = ssr ? opts.buildConfig.server : opts.wromoConfig.outDir;
...
async function generatePage(
	...
	if (!pageModule) {
		throw new Error(
			`Unable to find the module for ${pageData.component}. This is unexpected and likely a bug in Wromo, please report.`
		);
	}

	if (shouldSkipDraft(pageModule, opts.wromoConfig)) {
	...
	for (let i = 0; i < pageData.paths.length; i++) {
		...
		const filePath = getOutputFilename(opts.wromoConfig, path);
...
async function generatePath(
	...
	const { wromoConfig, logging, origin, routeCache } = opts;
	...
	const site =
		wromoConfig.base !== '/'
			? joinPaths(wromoConfig.site?.toString() || 'http://localhost/', wromoConfig.base)
			: wromoConfig.site;
	const links = createLinkStylesheetElementSet(linkIds.reverse(), site);
	const scripts = createModuleScriptsSet(hoistedScripts ? [hoistedScripts] : [], site);

	// Add all injected scripts to the page.
	for (const script of wromoConfig._ctx.scripts) {
	...
	const ssr = isBuildingToSSR(opts.wromoConfig);
	const url = new URL(opts.wromoConfig.base + removeLeadingForwardSlash(pathname), origin);
	const options: RenderOptions = {
		links,
		logging,
		markdown: wromoConfig.markdown,
		mod,
		origin,
		pathname,
		scripts,
		...
		async resolve(specifier: string) {
			...
			if (typeof hashedFilePath !== 'string') {
				// If no "wromo:scripts/before-hydration.js" script exists in the build,
			...
			return prependForwardSlash(npath.posix.join(wromoConfig.base, hashedFilePath));
		},
		request: createRequest({ url, headers: new Headers(), logging, ssr }),
		route: pageData.route,
		routeCache,
		site: wromoConfig.site
			? new URL(wromoConfig.base, wromoConfig.site).toString()
			: wromoConfig.site,
		ssr,
	};
	...
	const outFolder = getOutFolder(wromoConfig, pathname, pageData.route.type);
	const outFile = getOutFile(wromoConfig, outFolder, pathname, pageData.route.type);
