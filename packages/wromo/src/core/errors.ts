...
import type { SSRError } from '../@types/wromo';
...
export function fixViteErrorMessage(_err: unknown, server: ViteDevServer) {
	...
	// Fix: Wromo.glob() compiles to import.meta.glob() by the time Vite sees it,
	// so we need to update this error message in case it originally came from Wromo.glob().
	if (err.message === 'import.meta.glob() can only accept string literals.') {
		err.message = 'Wromo.glob() and import.meta.glob() can only accept string literals.';
	}
	return err;
}
...
function generateHint(err: ErrorWithMetadata): string | undefined {
	if (/Unknown file extension \"\.(jsx|vue|svelte|wromo|css)\" for /.test(err.message)) {
		return 'You likely need to add this package to `vite.ssr.noExternal` in your wromo config file.';
	} else {
...
export function collectErrorMetadata(e: any): ErrorWithMetadata {
	...
	// Wromo error (thrown by esbuild so it needs to be formatted for Vite)
