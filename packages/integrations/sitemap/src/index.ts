...
const PKG_NAME = '@wromojs/sitemap';
const OUTFILE = 'sitemap-index.xml';

const createPlugin = (options?: SitemapOptions): WromoIntegration => {
	let config: WromoConfig;
	return {
		name: PKG_NAME,

		hooks: {
			'wromo:config:done': async ({ config: cfg }) => {
				config = cfg;
			},

			'wromo:build:done': async ({ dir, pages }) => {
