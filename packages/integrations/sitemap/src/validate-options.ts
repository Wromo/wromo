...
export const validateOptions = (site: string | undefined, opts: SitemapOptions) => {
	...
	z.object({
		site: z.string().optional(), // Wromo takes care of `site`: how to validate, transform and refine
		...
		.refine((options) => options.site || options.canonicalURL, {
			message: 'Required `site` wromo.config option or `canonicalURL` integration option',
		})
		.parse({
			site,
