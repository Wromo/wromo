...
function getDefaultTailwindConfig(srcUrl: URL): TailwindConfig {
	return resolveConfig({
		...
		plugins: [],
		content: [path.join(fileURLToPath(srcUrl), `**`, `*.{wromo,html,js,jsx,svelte,ts,tsx,vue}`)],
...
export default function tailwindIntegration(options: TailwindOptions): WromoIntegration {
	const applyBaseStyles = options?.config?.applyBaseStyles ?? true;
	const customConfigPath = options?.config?.path;
	return {
		name: '@wromojs/tailwind',
		hooks: {
			'wromo:config:setup': async ({ config, injectScript }) => {
				...
				if (applyBaseStyles) {
					...
					injectScript('page-ssr', `import '@wromojs/tailwind/base.css';`);
				}
			},
		},
	};
