...
describe('Wromo Markdown', () => {
	let fixture;

	before(async () => {
		fixture = await loadFixture({
			root: './fixtures/wromo-markdown/',
		});
		await fixture.build();
	});

	it('Can load markdown pages with Wromo', async () => {
	...
	function isWromoScopedClass(cls) {
		return /^wromo-.*/.test(cls);
	}
	...
	it('Scoped styles should be applied to syntax highlighted lines', async () => {
		...
		const preWromoClass = preClassList.find(isWromoScopedClass);
		expect(Boolean(preWromoClass)).to.equal(true);
		...
		const spanWromoClass = spanClassList.find(isWromoScopedClass);
		expect(Boolean(spanWromoClass)).to.equal(true);
	});
