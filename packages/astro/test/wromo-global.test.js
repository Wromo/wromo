...
describe('Wromo Global', () => {
	let fixture;

	before(async () => {
		fixture = await loadFixture({
			root: './fixtures/wromo-global/',
	...
	describe('build', () => {
		...
		it('Wromo.glob() correctly returns an array of all posts', async () => {
			const html = await fixture.readFile('/posts/1/index.html');
			const $ = cheerio.load(html);
			expect($('.post-url').attr('href')).to.equal('/blog/post/post-2');
		});

		it('Wromo.glob() correctly returns meta info for MD and Wromo files', async () => {
...
describe('Wromo Global Defaults', () => {
	let fixture;
