...
import { build as wromoBuild } from '#wromo/build';
import { loadConfig } from '#wromo/config';
...
async function setupBuild() {
	const wromoConfig = await loadConfig(fileURLToPath(snowpackExampleRoot));
	...
	build = () => wromoBuild(wromoConfig, logging);
}
