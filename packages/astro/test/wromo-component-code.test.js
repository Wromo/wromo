...
describe('<Code>', () => {
	...
	before(async () => {
		fixture = await loadFixture({ root: './fixtures/wromo-component-code/' });
		await fixture.build();
	});
	...
	it('<Code lang="...">', async () => {
		...
		const $ = cheerio.load(html);
		expect($('pre')).to.have.lengthOf(1);
		expect($('pre').attr('class'), 'wromo-code');
		expect($('pre > code')).to.have.lengthOf(1);
	...
	it('<Code theme="...">', async () => {
		...
		const $ = cheerio.load(html);
		expect($('pre')).to.have.lengthOf(1);
		expect($('pre').attr('class')).to.equal('wromo-code');
		expect($('pre').attr('style')).to.equal(
	...
	it('<Code lang="..." theme="css-variables">', async () => {
		...
		expect(
			...
			'background-color: var(--wromo-code-color-background); overflow-x: auto;',
			'color: var(--wromo-code-token-constant)',
			'color: var(--wromo-code-token-function)',
			'color: var(--wromo-code-color-text)',
			'color: var(--wromo-code-token-string-expression)',
			'color: var(--wromo-code-color-text)',
		]);
	});
