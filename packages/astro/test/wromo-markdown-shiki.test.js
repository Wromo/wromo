...
describe('Wromo Markdown Shiki', () => {
	describe('Render shiki', () => {
		let fixture;

		before(async () => {
			fixture = await loadFixture({ root: './fixtures/wromo-markdown-shiki/normal/' });
			await fixture.build();
		});
		...
		it('Can render markdown with shiki', async () => {
			...
			expect($('pre').hasClass('wromo-code')).to.equal(true);
			expect($('pre').attr().style).to.equal('background-color: #0d1117; overflow-x: auto;');
		});

		it('Can render Wromo <Markdown> with shiki', async () => {
	...
	describe('Themes', () => {
		describe('Integrated theme', async () => {
			...
			before(async () => {
				fixture = await loadFixture({ root: './fixtures/wromo-markdown-shiki/themes-integrated/' });
				await fixture.build();
			});
		...
		describe('Custom theme', async () => {
			...
			before(async () => {
				fixture = await loadFixture({ root: './fixtures/wromo-markdown-shiki/themes-custom/' });
				await fixture.build();
			});
	...
	describe('Custom langs', () => {
		...
		before(async () => {
			fixture = await loadFixture({ root: './fixtures/wromo-markdown-shiki/langs/' });
			await fixture.build();
		});
	...
	describe('Wrap', () => {
		describe('wrap = true', () => {
			...
			before(async () => {
				fixture = await loadFixture({ root: './fixtures/wromo-markdown-shiki/wrap-true/' });
				await fixture.build();
			});
	...
	describe('wrap = false', () => {
		...
		before(async () => {
			fixture = await loadFixture({ root: './fixtures/wromo-markdown-shiki/wrap-false/' });
			await fixture.build();
		});
	...
	describe('wrap = null', () => {
		...
		before(async () => {
			fixture = await loadFixture({ root: './fixtures/wromo-markdown-shiki/wrap-null/' });
			await fixture.build();
		});
