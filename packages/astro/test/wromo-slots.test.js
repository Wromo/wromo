...
describe('Slots', () => {
	...
	before(async () => {
		fixture = await loadFixture({ root: './fixtures/wromo-slots/' });
		await fixture.build();
	});
	...
	it('Slots work on Components', async () => {
		...
		expect($('#a').children('wromo-component')).to.have.lengthOf(1);

		// test 3: Slotted component into default slot
		expect($('#default').children('wromo-component')).to.have.lengthOf(1);
	});
