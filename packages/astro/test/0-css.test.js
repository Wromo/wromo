...
describe('CSS', function () {
	...
	describe('build', () => {
		...
		describe('Wromo Styles', () => {
			...
			it('No <style> skips scoping', async () => {
				// Wromo component without <style> should not include scoped class
				expect($('#no-scope').attr('class')).to.equal(undefined);
			});

			it('Child inheritance', async () => {
				expect($('#passed-in').attr('class')).to.match(/outer wromo-[A-Z0-9]+ wromo-[A-Z0-9]+/);
			});

			it('Using hydrated components adds wromo-island styles', async () => {
				const inline = $('style').html();
				expect(inline).to.include('display:contents');
			});

			it('<style lang="sass">', async () => {
				expect(bundledCSS).to.match(new RegExp('h1.wromo-[^{]*{color:#90ee90}'));
			});

			it('<style lang="scss">', async () => {
				expect(bundledCSS).to.match(new RegExp('h1.wromo-[^{]*{color:#ff69b4}'));
			});
		});
	...
	describe('dev', () => {
		...
		it('resolves Wromo styles', async () => {
			const allInjectedStyles = $('style[data-wromo-injected]').text();

			expect(allInjectedStyles).to.contain('.linked-css.wromo-');
			expect(allInjectedStyles).to.contain('.linked-sass.wromo-');
			expect(allInjectedStyles).to.contain('.linked-scss.wromo-');
			expect(allInjectedStyles).to.contain('.wrapper.wromo-');
		});
