...
describe('Component Libraries', () => {
	...
	describe('build', async () => {
		...
		it('Built .wromo pages', async () => {
			let html = await fixture.readFile('/with-wromo/index.html');
		...
		it('Works with .wromo components', async () => {
			const html = await fixture.readFile('/with-wromo/index.html');
			const $ = cheerioLoad(html);
			...
			expect(await findEvidence('with-wromo/index.html')).to.equal(
				true,
				"Included the .wromo component's <style>"
			);
		});
		...
		it('Works with react components', async () => {
			...
			expect($('wromo-island[uid]')).to.have.lengthOf(1, 'Included one hydration island');
		});
	...
	describe('dev', async () => {
		...
		it('Works with .wromo components', async () => {
			const html = await fixture.fetch('/with-wromo/').then((res) => res.text());
			const $ = cheerioLoad(html);

			expect($('button').text()).to.equal('Click me', "Rendered the component's slot");

			const findEvidence = createFindEvidence(/border-radius:( )*1rem/);
			expect(await findEvidence('/with-wromo/')).to.equal(
				true,
				"Included the .wromo component's <style>"
			);
		});
