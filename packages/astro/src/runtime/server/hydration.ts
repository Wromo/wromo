...
export async function generateHydrateScript(
	...
	metadata: Required<WromoComponentMetadata>
): Promise<SSRElement> {
	const { renderer, result, wromoId, props } = scriptOptions;
	...
	island.props['before-hydration-url'] = await result.resolve('wromo:scripts/before-hydration.js');
