import { transform } from '@wromojs/compiler';
import { renderMarkdown } from '@wromojs/markdown-remark';
...
// TODO: Clean up some of the shared logic between this Markdown plugin and the Wromo plugin.
// Both end up connecting a `load()` hook to the Wromo compiler, and share some copy-paste
// logic in how that is done.
export default function markdown({ config }: WromoPluginOptions): Plugin {
	...
	return {
		...
		async resolveId(id, importer, options) {
			...
			// when fetching and then filtering many markdown files, like with import.meta.glob() or Wromo.glob().
		...
		async load(id) {
			...
			if (id.endsWith(`.md${MARKDOWN_IMPORT_FLAG}`)) {
				...
				return {
					code: `
						...
						Content.isWromoComponentFactory = true;
			...
			// This returns the compiled markdown -> wromo component that renders to HTML.
			if (id.endsWith('.md')) {
				...
				let { code: wromoResult, metadata } = renderResult;
				const { layout = '', components = '', setup = '', ...content } = frontmatter;
				content.wromo = metadata;
				const prelude = `---
import { slug as $$slug } from '@wromojs/markdown-remark/ssr-utils';
...
${setup}`.trim();
				...
				// Transform from `.wromo` to valid `.ts`
				let transformResult = await transform(wromoResult, {
