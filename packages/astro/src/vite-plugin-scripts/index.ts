...
import { WromoConfig, InjectedScriptStage } from '../@types/wromo.js';
...
// resolve correctly.
const SCRIPT_ID_PREFIX = `wromo:scripts/`;
...
export default function wromoScriptsPlugin({ config }: { config: WromoConfig }): VitePlugin {
	return {
		name: 'wromo:scripts',
		async resolveId(id) {
		...
		buildStart(options) {
			...
			// `wromo/client/*` input, which signifies both conditions are met.
			const hasHydratedComponents =
				Array.isArray(options.input) &&
				options.input.some((input) => input.startsWith('wromo/client'));
