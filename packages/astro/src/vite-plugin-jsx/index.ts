...
import type { WromoConfig, WromoRenderer } from '../@types/wromo';
...
const JSX_RENDERER_CACHE = new WeakMap<WromoConfig, Map<string, WromoRenderer>>();
const JSX_EXTENSIONS = new Set(['.jsx', '.tsx']);
const IMPORT_STATEMENTS: Record<string, string> = {
	react: "import React from 'react'",
	preact: "import { h } from 'preact'",
	'solid-js': "import 'solid-js/web'",
	wromo: "import 'wromo/jsx-runtime'",
};
...
function collectJSXRenderers(renderers: WromoRenderer[]): Map<string, WromoRenderer> {
	const renderersWithJSXSupport = renderers.filter((r) => r.jsxImportSource);
	return new Map(
		renderersWithJSXSupport.map((r) => [r.jsxImportSource, r] as [string, WromoRenderer])
	);
}
...
interface WromoPluginJSXOptions {
	config: WromoConfig;
	logging: LogOptions;
}

/** Use Wromo config to allow for alternate or multiple JSX renderers (by default Vite will assume React) */
export default function jsx({ config, logging }: WromoPluginJSXOptions): Plugin {
			...
			if (!jsxRenderers) {
				...
				if (possibleRenderers.size === 0) {
					...
					throw new Error(
						`${colors.yellow(
							id
						)}\nUnable to resolve a JSX renderer! Did you forget to include one? Add a JSX integration like \`@wromojs/react\` to your \`wromo.config.mjs\` file.`
					);
				}
			...
			if (importSource) {
				...
				if (!jsxRenderer) {
					error(
						...
						`${colors.yellow(
							id
						)} No renderer installed for ${importSource}. Try adding \`@wromojs/${importSource}\` to your project.`
					);
					return null;
				}
				// downlevel any non-standard syntax, but preserve JSX
				...
				return await transformJSX({
					code: jsxCode,
					id,
					renderer: jsxRenderers.get(importSource) as WromoRenderer,
					mode,
					ssr,
				});
			}
