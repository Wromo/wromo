...
// If we throw at the top-level, international users can't use Wromo.
//
...
/** Emit a error message, Useful when Wromo can't recover from some error. */
...
export function debug(...args: any[]) {
	if ('_wromoGlobalDebug' in globalThis) {
		(globalThis as any)._wromoGlobalDebug(...args);
	}
}
 ...
 */
export function warnIfUsingExperimentalSSR(opts: LogOptions, config: WromoConfig) {
