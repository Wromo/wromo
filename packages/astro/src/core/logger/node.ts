 ...
 * Wromo (along with many of its dependencies) uses the `debug` package for debug logging.
 * You can enable these logs with the `DEBUG=wromo:*` environment variable.
 * More info https://github.com/debug-js/debug#environment-variables
 */
export function debug(type: string, ...messages: Array<any>) {
	const namespace = `wromo:${type}`;
...
(globalThis as any)._wromoGlobalDebug = debug;
...
export function enableVerboseLogging() {
	...
	debug(
		'cli',
		'Tip: Set the DEBUG env variable directly for more control. Example: "DEBUG=wromo:*,vite:* wromo build".'
	);
}
