...
export async function getParamsAndProps(
	...
	if (route && !route.pathname) {
		...
		// Ex. console.log(Wromo.props) -> {}, but console.log(Wromo.props.property) -> 'expected value'
...
export async function render(opts: RenderOptions): Promise<Response> {
	...
	if (!Component)
		throw new Error(`Expected an exported Wromo component but received typeof ${typeof Component}`);
	...
	if (!Component.isWromoComponentFactory) {
