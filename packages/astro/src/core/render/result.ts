import type { MarkdownRenderingOptions } from '@wromojs/markdown-remark';
...
export function createResult(args: CreateResultArgs): SSRResult {
	...
	const result: SSRResult = {
		...
		/** This function returns the `Wromo` faux-global */
		createWromo(
			wromoGlobal: WromoGlobalPartial,
			props: Record<string, any>,
			slots: Record<string, any> | null
		) {
			const wromoSlots = new Slots(result, slots);

			const Wromo = {
				__proto__: wromoGlobal,
				canonicalURL,
				params,
				props,
				request,
				redirect: args.ssr
					...
					: onlyAvailableInSSR('Wromo.redirect'),
				resolve(path: string) {
...
${extra}`
				...
				response,
				slots: wromoSlots,
			} as unknown as WromoGlobal;

			Object.defineProperty(Wromo, '__renderMarkdown', {
				...
				value: async function (content: string, opts: MarkdownRenderingOptions) {
					...
					if (!renderMarkdown) {
						...
						let wromoRemark = '@wromojs/markdown-remark';

						renderMarkdown = (await import(wromoRemark)).renderMarkdown;
					}
