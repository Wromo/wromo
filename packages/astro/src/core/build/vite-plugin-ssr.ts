...
import type { WromoAdapter } from '../../@types/wromo';
...
export const virtualModuleId = '@wromojs-ssr-virtual-entry';
const resolvedVirtualModuleId = '\0' + virtualModuleId;
const manifestReplace = '@@WROMO_MANIFEST_REPLACE@@';
const replaceExp = new RegExp(`['"](${manifestReplace})['"]`, 'g');

export function vitePluginSSR(
	buildOpts: StaticBuildOptions,
	internals: BuildInternals,
	adapter: WromoAdapter
): VitePlugin {
	return {
		name: '@wromojs/vite-plugin-wromo-ssr',
		enforce: 'post',
		options(opts) {
...
import { deserializeManifest as _deserializeManifest } from 'wromo/app';
...
export async function injectManifest(buildOpts: StaticBuildOptions, internals: BuildInternals) {
	...
	await runHookBuildSsr({ config: buildOpts.wromoConfig, manifest });
...
function buildManifest(
	...
	for (const pageData of eachPageData(internals)) {
		...
		routes.push({
			...
			scripts: [
				...scripts,
				...wromoConfig._ctx.scripts
					.filter((script) => script.stage === 'head-inline')
					.map(({ stage, content }) => ({ stage, children: content })),
			],
			routeData: serializeRouteData(pageData.route, wromoConfig.trailingSlash),
		});
	}
