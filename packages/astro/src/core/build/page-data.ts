...
import type { WromoConfig, ComponentInstance, ManifestData, RouteData } from '../../@types/wromo';
...
export async function collectPagesData(
	...
	const { wromoConfig, logging, manifest, origin, routeCache, viteServer } = opts;

	const assets: Record<string, string> = {};
	const allPages: AllPagesData = {};
	const builtPaths = new Set<string>();

	const buildMode = isBuildingToSSR(wromoConfig) ? 'ssr' : 'static';
		...
		if (route.pathname) {
			const routeCollectionLogTimeout = setInterval(() => {
				info(
					...
					`${colors.bold(
						route.component
					)} is taking a bit longer to import. This is common for larger "Wromo.glob(...)" or "import.meta.globEager(...)" calls, for instance. Hang tight!`
				);
...
async function getStaticPathsForRoute(
	...
	const { wromoConfig, logging, routeCache, ssr, viteServer } = opts;
	if (!viteServer) throw new Error(`vite.createServer() not called!`);
	const filePath = new URL(`./${route.component}`, wromoConfig.root);
