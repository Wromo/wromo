...
export function devStart({
	...
	const messages = [
		`${emoji('🚀 ', '')}${bgGreen(black(` wromo `))} ${green(`v${version}`)} ${dim(
...
export function telemetryNotice() {
	const headline = yellow(`Wromo now collects ${bold('anonymous')} usage data.`);
	const why = `This ${bold('optional program')} will help shape our roadmap.`;
	const more = `For more info, visit ${underline('https://wromo.build/telemetry')}`;
...
export function prerelease({ currentVersion }: { currentVersion: string }) {
	...
	const warning = `  Feedback? ${underline('https://wromo.build/issues')}`;
...
export function formatConfigErrorMessage(err: ZodError) {
	...
	return `${red('[config]')} Wromo found issue(s) with your configuration:\n${errorList.join(
		'\n'
	)}`;
}
...
export function printHelp({
	...
	if (commands) {
		message.push(
			linebreak(),
			title('Commands'),
			table(commands, { padding: 28, prefix: `  ${commandName || 'wromo'} ` })
		);
	}
