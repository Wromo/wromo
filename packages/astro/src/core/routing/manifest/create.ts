import type { WromoConfig, ManifestData, RouteData, RoutePart } from '../../../@types/wromo';
...
function getPattern(segments: RoutePart[][], addTrailingSlash: WromoConfig['trailingSlash']) {
...
function getTrailingSlashPattern(addTrailingSlash: WromoConfig['trailingSlash']): string {
...
function validateSegment(segment: string, file = '') {
	...
	if (/^\$/.test(segment)) {
		throw new Error(
			`Invalid route ${file} \u2014 Wromo's Collections API has been replaced by dynamic route params.`
		);
	}
	if (/\]\[/.test(segment)) {
...
export function createRouteManifest(
	{ config, cwd }: { config: WromoConfig; cwd?: string },
	logging: LogOptions
): ManifestData {
	...
	function walk(dir: string, parentSegments: RoutePart[][], parentParams: string[]) {
		...
		fs.readdirSync(dir).forEach((basename) => {
			...
			// filter out "foo.wromo_tmp" files, etc
	...
	if (fs.existsSync(pages)) {
		...
		warn(logging, 'wromo', `Missing pages directory: ${pagesDirRootRelative}`);
	}
	...
	config?._ctx?.injectedRoutes?.forEach(({ pattern: name, entryPoint }) => {
		...
		const type = resolved.endsWith('.wromo') ? 'page' : 'endpoint';
