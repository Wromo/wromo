...
import type { PluginMetadata as WromoPluginMetadata } from './types';
...
import { parseWromoRequest, ParsedRequestResult } from './query.js';
...
/** Transform .wromo files for Vite */
export default function wromo({ config, logging }: WromoPluginOptions): vite.Plugin {
	...
	function resolveRelativeFromWromoParent(id: string, parsedFrom: ParsedRequestResult): string {
	...
	return {
		...
		// note: don’t claim .wromo files with resolveId() — it prevents Vite from transpiling the final JS (import.meta.globEager, etc.)
		async resolveId(id, from, opts) {
			// If resolving from an wromo subresource such as a hoisted script,
			// we need to resolve relative paths ourselves.
			if (from) {
				const parsedFrom = parseWromoRequest(from);
				const isWromoScript = parsedFrom.query.wromo && parsedFrom.query.type === 'script';
				if (isWromoScript && isRelativePath(id)) {
					return this.resolve(resolveRelativeFromWromoParent(id, parsedFrom), from, {
						custom: opts.custom,
						skipSelf: true,
					});
				}
			...
			// serve sub-part requests (*?wromo) as virtual modules
			const { query } = parseWromoRequest(id);
			if (query.wromo) {
				// Convert /src/pages/index.wromo?wromo&type=style to /Users/name/
		...
		async load(this: PluginContext, id, opts) {
			const parsedId = parseWromoRequest(id);
			const query = parsedId.query;
			if (!id.endsWith('.wromo') && !query.wromo) {
				return null;
			}
			...
			if (query.wromo) {
				if (query.type === 'style') {
					if (typeof query.index === 'undefined') {
						throw new Error(`Requests for Wromo CSS must include an index.`);
					}
			...
			try {
				...
				if (!resolvedConfig.isProduction) {
					...
					while (i < transformResult.scripts.length) {
						deps.add(`${id}?wromo&type=script&index=${i}&lang.ts`);
						SUFFIX += `import "${id}?wromo&type=script&index=${i}&lang.ts";`;
						i++;
					}
				...
				const wromoMetadata: WromoPluginMetadata['wromo'] = {
				...
				if (err.stack.includes('wasm-function')) {
					const search = new URLSearchParams({
						labels: 'compiler',
						title: '🐛 BUG: `@wromojs/compiler` panic',
						template: '---01-bug-report.yml',
						'bug-description': `\`@wromojs/compiler\` encountered an unrecoverable error when compiling the following file.
