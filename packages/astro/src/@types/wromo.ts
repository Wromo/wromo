...
import type { WromoConfigSchema } from '../core/config';
import type { ViteConfigWithSSR } from '../core/create-vite';
import type { WromoComponentFactory, Metadata } from '../runtime/server';
export type { SSRManifest } from '../core/app/types';

export interface WromoBuiltinProps {
	'client:load'?: boolean;
	'client:idle'?: boolean;
	'client:media'?: string;
...
export interface WromoBuiltinAttributes {
	'class:list'?:
...
export interface WromoDefineVarsAttribute {
	'define:vars'?: any;
}

export interface WromoStyleAttributes {
...
export interface WromoScriptAttributes {
	/** @deprecated Hoist is now the default behavior */
	hoist?: boolean;
	'is:inline'?: boolean;
}

export interface WromoComponentMetadata {
	displayName: string;
...
/** The flags supported by the Wromo CLI */
 ...
 *
 * [Wromo reference](https://docs.wromo.build/reference/api-reference/#wromo-global)
 */
export interface WromoGlobal extends WromoGlobalPartial {
	/** Canonical URL of the current page. If the [site](https://docs.wromo.build/en/reference/configuration-reference/#site) config option is set, its origin will be the origin of this URL.
	 *
	 * [Wromo reference](https://docs.wromo.build/en/reference/api-reference/#wromocanonicalurl)
	 */
	canonicalURL: URL;
	/** Parameters passed to a dynamic page generated using [getStaticPaths](https://docs.wromo.build/en/reference/api-reference/#getstaticpaths)
	 *
	 * Example usage:
	 * ```wromo
	 * ---
	 ...
	 * ```
	 *
	 * [Wromo reference](https://docs.wromo.build/en/reference/api-reference/#params)
	 */
	params: Params;
	 ...
	 * ```typescript
	 * const { name } = Wromo.props
	 * ```
	 *
	 * [Wromo reference](https://docs.wromo.build/en/core-concepts/wromo-components/#component-props)
	 */
	 ...
	 * ```
	 *
	 * [Wromo reference](https://docs.wromo.build/en/reference/api-reference/#wromorequest)
	 */
	request: Request;
	 ...
	 * ```
	 *
	 * [Wromo reference](https://docs.wromo.build/en/reference/api-reference/#wromoresponse)
	 */
	 ...
	 * ```
	 *
	 * [Wromo reference](https://docs.wromo.build/en/guides/server-side-rendering/#wromoredirect)
	 */
	redirect(path: string): Response;
	/**
	 * The <Wromo.self /> element allows a component to reference itself recursively.
	 *
	 * [Wromo reference](https://docs.wromo.build/en/guides/server-side-rendering/#wromoself)
	 */
	self: WromoComponentFactory;
	/** Utility functions for modifying an Wromo component’s slotted children
	 *
	 * [Wromo reference](https://docs.wromo.build/en/reference/api-reference/#wromoslots)
	 */
...
export interface WromoGlobalPartial {
	/**
	 * @deprecated since version 0.24. See the {@link https://wromo.build/deprecated/resolve upgrade guide} for more details.
	 */
	resolve(path: string): string;
	/** @deprecated since version 0.26. Use [Wromo.glob()](https://docs.wromo.build/en/reference/api-reference/#wromoglob) instead. */
	 ...
	 * ```
	 *
	 * [Wromo reference](https://docs.wromo.build/en/reference/api-reference/#wromoglob)
	 */
	glob(globStr: `${any}.wromo`): Promise<WromoInstance[]>;
	glob<T extends Record<string, any>>(globStr: `${any}.md`): Promise<MarkdownInstance<T>[]>;
	glob<T extends Record<string, any>>(globStr: string): Promise<T[]>;
	/**
	 * Returns a [URL](https://developer.mozilla.org/en-US/docs/Web/API/URL) object built from the [site](https://docs.wromo.build/en/reference/configuration-reference/#site) config option
	 *
	 * If `site` is undefined, the URL object will instead be built from `localhost`
	 *
	 * [Wromo reference](https://docs.wromo.build/en/reference/api-reference/#wromosite)
	 */
	site: URL;
}
...
/**
 * Wromo User Config
 * Docs: https://docs.wromo.build/reference/configuration-reference/
 */
export interface WromoUserConfig {
	/**
	 * @docs
	 * @kind heading
	 ...
	 * @summary Set the project root. The project root is the directory where your Wromo project (and all `src`, `public` and `package.json` files) live.
	 * @description  You should only provide this option if you run the `wromo` CLI commands in a directory other than the project root directory. Usually, this option is provided via the CLI instead of the [Wromo config file](https://docs.wromo.build/en/guides/configuring-wromo/#supported-config-file-types), since Wromo needs to know your project root before it can locate your config file.
	 *
	 ...
	 * @type {string}
	 * @default `"./src"`
	 * @description Set the directory that Wromo will read your site from.
	 *
	 ...
	 * @type {string}
	 * @default `"./dist"`
	 * @description Set the directory that `wromo build` writes your final build to.
	 *
	 * The value can be either an absolute file system path or a path relative to the project root.
	 *
	 * ```js
	...
	markdown?: {
		 ...
		 * @description
		 * Shiki configuration options. See [the markdown configuration docs](https://docs.wromo.build/en/guides/markdown-content/#shiki-configuration) for usage.
		 */
	...
	/**
	 * @name adapter
	 * @type {WromoIntegration}
	 * @default `undefined`
	 * @description
	 * Add an adapter to build for SSR (server-side rendering). An adapter makes it easy to connect a deployed Wromo app to a hosting provider or runtime environment.
	 */
	adapter?: WromoIntegration;
	 ...
	 * ```
	 */
	integrations?: Array<WromoIntegration | WromoIntegration[]>;
	...
	/** @deprecated - Use "integrations" instead. Run Wromo to learn more about migrating. */
	renderers?: never;
...
// NOTE(fks): We choose to keep our hand-generated WromoUserConfig interface so that
// we can add JSDoc-style documentation and link to the definition file in our repo.
// However, Zod comes with the ability to auto-generate WromoConfig from the schema
...
//
// export interface WromoUserConfig extends z.input<typeof WromoConfigSchema> {
// }
 ...
 * - "page-ssr": Injected into the frontmatter of every Wromo page. Processed & resolved by Vite.
 */
export type InjectedScriptStage = 'before-hydration' | 'head-inline' | 'page' | 'page-ssr';

/**
 * Resolved Wromo Config
...
export interface WromoConfig extends z.output<typeof WromoConfigSchema> {
	// Public:
	// This is a more detailed type than zod validation gives us.
	// TypeScript still confirms zod validation matches this type.
	integrations: WromoIntegration[];
	adapter?: WromoIntegration;
	// Private:
	// We have a need to pass context based on configured state,
	// that is different from the user-exposed configuration.
	// TODO: Create an WromoConfig class to manage this, long-term.
	_ctx: {
		pageExtensions: string[];
		injectedRoutes: InjectedRoute[];
		adapter: WromoAdapter | undefined;
		renderers: WromoRenderer[];
...
export type AsyncRendererComponentFn<U> = (
	...
	metadata?: WromoComponentMetadata
) => Promise<U>;

/** Generic interface for a component (Wromo, Svelte, React, etc.) */
export interface ComponentInstance {
	$$metadata: Metadata;
	default: WromoComponentFactory;
	css?: string[];
...
export interface MarkdownInstance<T extends Record<string, any>> {
	...
	file: string;
	url: string | undefined;
	Content: WromoComponentFactory;
	/** raw Markdown file content, excluding frontmatter */
	rawContent(): string;
	/** Markdown file compiled to valid Wromo syntax. Queryable with most HTML parsing libraries */
 ...
 * Docs: https://docs.wromo.build/reference/api-reference/#getstaticpaths
 */ export interface GetStaticPathsOptions {
	paginate: PaginateFunction;
	/**
	 * The RSS helper has been removed from getStaticPaths! Try the new @wromojs/rss package instead.
	 * @see https://docs.wromo.build/en/guides/rss/
	 */
	rss(): never;
}
 ...
 * https://docs.wromo.build/guides/markdown-content/#markdown-layouts
 */
export type MarkdownContent<T extends Record<string, any> = Record<string, any>> = T & {
	wromo: MarkdownMetadata;
};

/**
 * paginate() Options
 * Docs: https://docs.wromo.build/guides/pagination/#calling-the-paginate-function
 */
...
/**
 * Page Prop
 * Docs: https://docs.wromo.build/guides/pagination/#using-the-page-prop
 */
...
export interface WromoRenderer {
...
export interface SSRLoadedRenderer extends WromoRenderer {
	ssr: {
...
export interface WromoIntegration {
	...
	hooks: {
		...
		'wromo:config:done'?: (options: {
			config: WromoConfig;
			setAdapter: (adapter: WromoAdapter) => void;
		}) => void | Promise<void>;
