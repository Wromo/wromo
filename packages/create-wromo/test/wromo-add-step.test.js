...
describe('[create-wromo] wromo add', function () {
	...
	it('should use "wromo add" when user has installed dependencies', function () {
		...
		return promiseWithTimeout((resolve) => {
			...
			stdout.on('data', (chunk) => {
				...
				if (chunk.includes(PROMPT_MESSAGES.wromoAdd('wromo add --yes'))) {
					resolve();
				}
			});
		});
	...
	it('should use "npx wromo@latest add" when use has NOT installed dependencies', function () {
		const { stdout, stdin } = setup([tempDir]);
		return promiseWithTimeout((resolve) => {
			const seen = new Set();
			const installPrompt = PROMPT_MESSAGES.install('npm');
			stdout.on('data', (chunk) => {
				...
				if (chunk.includes(PROMPT_MESSAGES.wromoAdd('npx wromo@latest add --yes'))) {
					resolve();
				}
			});
		});
