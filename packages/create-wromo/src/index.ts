...
// fixes the issue so that create-wromo now works on all npm version.
...
const FILES_TO_REMOVE = ['.stackblitzrc', 'sandbox.config.json', 'CHANGELOG.md']; // some files are only needed for online editors when using wromo.new. Remove for create-wromo installs.

export async function main() {
	const pkgManager = pkgManagerFromUserAgent(process.env.npm_config_user_agent);

	logger.debug('Verbose logging turned on');
	console.log(`\n${bold('Welcome to Wromo!')} ${gray(`(create-wromo v${version})`)}`);
	...
	const wromoAddCommand = installResponse.install
		? 'wromo add --yes'
		: `${pkgManagerExecCommand(pkgManager)} wromo@latest add --yes`;

	const wromoAddResponse = await prompts({
		type: 'confirm',
		name: 'wromoAdd',
		message: `Run "${wromoAddCommand}?" This lets you optionally add component frameworks (ex. React), CSS frameworks (ex. Tailwind), and more.`,
		initial: true,
	});
	...
	if (!wromoAddResponse.wromoAdd) {
		ora().info(
			`No problem. You can always run "${pkgManagerExecCommand(pkgManager)} wromo add" later!`
		);
	}

	if (wromoAddResponse.wromoAdd && !args.dryrun) {
		await execaCommand(
			wromoAddCommand,
			wromoAddCommand === 'wromo add --yes'
	...
	console.log(
		`Run ${bold(cyan(devCmd))} to start the Wromo dev server. ${bold(cyan('CTRL-C'))} to close.`
	);
	if (!installResponse.install) {
		console.log(yellow(`Remember to install dependencies first!`));
	}
	console.log(`\nStuck? Come join us at ${bold(cyan('https://wromo.build/chat'))}`);
}
